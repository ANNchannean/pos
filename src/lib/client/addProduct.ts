...
export function addProduct(para: ProductOrder) {
   ...
   if (product_order.some((e) => e.product_id === para.product_id)) {
      // Function ស្វែងរកទំនិញដែលមានហើយបូក ១បន្ថែម
      const obj = store.productOrders?.find((obj) => obj.product_id === para.product_id);
      if (obj) {
         obj.qty = obj.qty + 1;
...
export function modalDiscount(product_id: number) {
   ...
   const found = store.productOrders.find((e) => e.product_id === product_id);
   if (found) {
   ...
   document.getElementById('close_modalid'.concat(product_id.toString()))?.click();
}